...
export const RUNTIME_ADAPTER_CATALOG: RuntimeAdapterOption[] = [
  ...
  },
  {
    ...
    description: "Global card, wallet, subscription, metered billing, invoices, tax, and webhook-backed entitlement automation.",
    capabilities: ["checkout", "subscriptions", "metered-billing", "tax", "webhooks", "customer-portal"],
    requiredSecretRefs: ["STRIPE_SECRET_KEY", "STRIPE_WEBHOOK_SECRET"],
    edgeReady: true,
    nativeReady: true,
    selfHostReady: false,
    score: 94,
    configDefaults: { mode: "subscription-first", tax: "automatic", portal: true, webhookPath: "/api/public/webhooks/payments" },
  },
  {
    category: "payments",
    provider: "paddle",
    displayName: "Paddle Merchant of Record",
    description: "Merchant-of-record payments for SaaS teams that need global sales tax, invoices, subscriptions, and checkout localization.",
    capabilities: ["merchant-of-record", "subscriptions", "tax", "localized-checkout", "webhooks"],
    requiredSecretRefs: ["PADDLE_API_KEY", "PADDLE_WEBHOOK_SECRET"],
    edgeReady: true,
    nativeReady: true,
    selfHostReady: false,
    score: 89,
    configDefaults: { environment: "production", taxMode: "merchant-of-record", webhookPath: "/api/public/webhooks/payments" },
  },
  {
    category: "payments",
    provider: "razorpay",
    displayName: "Razorpay",
    description: "India-first payment stack for cards, UPI, subscriptions, mandates, invoices, and payout-connected products.",
    capabilities: ["upi", "cards", "subscriptions", "mandates", "invoices", "webhooks"],
    requiredSecretRefs: ["RAZORPAY_KEY_ID", "RAZORPAY_KEY_SECRET", "RAZORPAY_WEBHOOK_SECRET"],
    edgeReady: true,
    nativeReady: true,
    selfHostReady: false,
    score: 82,
    configDefaults: { currency: "INR", recurring: true, webhookPath: "/api/public/webhooks/payments" },
  },
  {
    category: "email",
    provider: "resend",
    displayName: "Resend Email",
    description: "Developer-friendly transactional email with domain verification, templates, analytics, and edge-compatible delivery.",
    capabilities: ["transactional", "templates", "domain-auth", "analytics", "webhooks"],
    requiredSecretRefs: ["RESEND_API_KEY", "EMAIL_FROM"],
    edgeReady: true,
    nativeReady: true,
    selfHostReady: false,
    score: 92,
    configDefaults: { bounceHandling: true, templateEngine: "react-email", webhookPath: "/api/public/webhooks/email" },
  },
  {
    category: "email",
    provider: "postmark",
    displayName: "Postmark",
    description: "High-deliverability transactional email with message streams, inbound parse, bounce webhooks, and strict sender reputation controls.",
    capabilities: ["transactional", "message-streams", "inbound", "bounce-webhooks", "templates"],
    requiredSecretRefs: ["POSTMARK_SERVER_TOKEN", "EMAIL_FROM"],
    edgeReady: true,
    nativeReady: true,
    selfHostReady: false,
    score: 88,
    configDefaults: { stream: "outbound", inbound: false, webhookPath: "/api/public/webhooks/email" },
  },
  {
    category: "email",
    provider: "smtp-self-host",
    displayName: "SMTP Self-host",
    description: "Portable SMTP delivery for private mail infrastructure, enterprise relays, or fully self-hosted deployments.",
    capabilities: ["smtp", "self-host", "private-relay", "templates"],
    requiredSecretRefs: ["SMTP_HOST", "SMTP_USER", "SMTP_PASSWORD", "EMAIL_FROM"],
    edgeReady: false,
    nativeReady: true,
    selfHostReady: true,
    score: 76,
    configDefaults: { port: 587, secure: true, pool: true },
  },
  {
    category: "push",
    provider: "fcm-apns",
    displayName: "FCM + APNs",
    description: "Native push pipeline for iOS, Android, and web with token registration, segments, campaigns, and delivery receipts.",
    capabilities: ["ios", "android", "web", "segments", "campaigns", "receipts"],
    requiredSecretRefs: ["FCM_SERVICE_ACCOUNT_JSON", "APNS_KEY_ID", "APNS_TEAM_ID", "APNS_PRIVATE_KEY"],
    edgeReady: true,
    nativeReady: true,
    selfHostReady: false,
    score: 90,
    configDefaults: { tokenStrategy: "per-device", quietHours: true, receiptTracking: true },
  },
  {
    category: "push",
    provider: "onesignal",
    displayName: "OneSignal",
    description: "Managed multi-channel push and in-app messaging with audience segments, journeys, and cross-platform SDK coverage.",
    capabilities: ["ios", "android", "web", "segments", "journeys", "in-app"],
    requiredSecretRefs: ["ONESIGNAL_APP_ID", "ONESIGNAL_REST_API_KEY"],
    edgeReady: true,
    nativeReady: true,
    selfHostReady: false,
    score: 86,
    configDefaults: { channels: ["push", "in-app"], journeyAutomation: true },
  },
  {
    category: "push",
    provider: "web-push-vapid",
    displayName: "Web Push VAPID",
    description: "Self-hostable browser push using VAPID keys, service workers, queue-backed fanout, and portable subscription storage.",
    capabilities: ["web", "vapid", "self-host", "service-worker", "queue-fanout"],
    requiredSecretRefs: ["VAPID_PUBLIC_KEY", "VAPID_PRIVATE_KEY", "VAPID_SUBJECT"],
    edgeReady: true,
    nativeReady: false,
    selfHostReady: true,
    score: 79,
    configDefaults: { serviceWorkerPath: "/sw.js", batchSize: 500, retryPolicy: "exponential" },
  },
];
