...
AS $$
  SELECT date_trunc('day', occurred_at)::date AS day, event_name, count(*)::bigint AS count
...
AS $$
  SELECT e.variant AS variant,
    ...
    CASE WHEN COUNT(*) = 0 THEN 0 ELSE ROUND(((COUNT(*) FILTER (WHERE e.is_conversion))::numeric * 100) / COUNT(*)::numeric, 4) END AS conversion_rate,
...
LANGUAGE sql STABLE SECURITY INVOKER SET search_path = public
AS $$
  SELECT e.metric_key AS metric_key, COALESCE(SUM(e.quantity),0)::numeric AS total, COUNT(*)::bigint AS event_count
  ...
  ORDER BY COALESCE(SUM(e.quantity),0)::numeric DESC;
$$;
